...
impl<'a, T> RectIterator<'a, T> {
    ...
    pub fn from_rect(buf: &'a [T], stride: usize, r: &Rect) -> RectIterator<'a, T> {
        RectIterator::new(
            buf,
            stride,
            r.x() as usize,
...
impl<'a, T> MutableRectIterator<'a, T> {
    ...
    pub fn from_rect(buf: &'a mut [T], stride: usize, r: &Rect) -> MutableRectIterator<'a, T> {
    ...
    pub fn area(&self) -> usize {
        (self.last_row - self.row) * (self.x1 - self.x0)
    }
}
...
impl<'a, T> Iterator for MutableRectIterator<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.row != self.last_row {
            ...
            self.buf = rest;
            Some((&mut slice[self.x0..self.x1], row))
        } else {
            None
        }
    }
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        let len = self.last_row - self.row;
        (len, Some(len))
    }
}
